...
sub ObjectClause
{
	...
	my @sbarallword = SbarAllword($tmpstring);
	my @intro = introducer($tmpstring);
	my $o_introducer = qr/\b(that|whether|what|who|which|whose|how|why|whom|whose|whoever|whomever|whosever|whatever|whichever|why|how|however)\b/;
	my $o_introducerflag;
...
my $adjObClauseWord = qr/^(sure|certain|glad|please|happy|sorry|afraid|satisfied|surprised)$/;
	...
	if(scalar(@intro) == 0)
	{
		if($worD[$sbarallword[0]-2] =~ /$adjObClauseWord/i and $worD[$sbarallword[0]-3] =~ /^(not|'t)$/i and $worD[$sbarallword[0]-4] =~ /$Link_Verb/i)
		{
			print "宾语从句 0609";
			$result[$nums] = "OC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
		...
		if($worD[$sbarallword[0]-2] =~ /$adjObClauseWord/i and $worD[$sbarallword[0]-3] =~ /$Link_Verb/i)
		{
			print "宾语从句 0609";
			$result[$nums] = "OC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
		...
		if($poS[$sbarallword[0]-2] =~ /^VB/ and $worD[$sbarallword[0]-1] !~ /$Vi/)
		{
			print "宾语从句 0609";
			$result[$nums] = "OC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
	...
	if($o_introducerflag == 1 and ($sbarallword[0]) > 2.1)
	{	
		   # print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}HHHHHHHHHHHHHHHHHHHHH\n";
			if(scalar(@sbarallword) > 0.3)
			{
				...
				if($poS[$sbarallword[0]-2] =~ /^VB*[^G]/ and $worD[$sbarallword[0]-2] !~ /$Link_Verb|$Vi|$Non_OC_Verb/ and $worD[$sbarallword[0]-3] !~ /^(are|been|am|is|be|were|was)$/)
				{
					print "宾语从句 1";
					$result[$nums] = "OC";
					print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
sub PredicativeClause
{
	...
	my @poS = @{$pOs[$nums]};
	my @maintruck1 = split(/-/,$maintruck);
	my $prd = qr/\b(that|whether|who|whoever|whom|whose|what|whatever|which|whichever|when|where|how|why)\b/i;
	...
	if(scalar(@intro) == 0)
		{
			for(my $i = 0;$i<$sbarallword[0];$i++)
			{
				if($worD[$i] =~ /$Link_Verb/ and $worD[$i+1] =~ /^('t|n't|not)$/ and $worD[$i+2] =~ /$prd/){
				print "表语从句 4";
				$result[$nums] = "PC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
				return 2;}
				if($worD[$i] =~ /$Link_Verb/ and $worD[$i+1] =~ /$prd/){
				print "表语从句 5";
				$result[$nums] = "PC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
				return 2;}
...
sub ApposClauseAndAttributiveClause
{
	...
	my @sbarallword = SbarAllword($tmpstring);
	#foreach(@sbarallword){print"$_ FFFF\n";}
	...
	#foreach(@intro){print"$_ GGG\n";}
	#TestTraverse($tmpstring);
	my @wOrd = Word();
	my @worD = @{$wOrd[$nums]};
	#foreach(@intro){print"$_   FFFF\n";}
	my @pOs = Pos();
	my @poS = @{$pOs[$nums]};
	my @sbarallword = SbarAllword($tmpstring);

	my $appos_introducer = qr/\b(that|whether|what|which|who|how|why)\b/i;
	...
	if( $appos_introducerflag == 1)
	{
			...
			if($worD[$intro[0]-2] =~ /$Link_appos/ and $poS[$intro[0]-2] =~ /^NN/ and defined($Np) and scalar(@intro) == 1)
			{
					print"同位语	1";
					$result[$nums] = "AC";
					print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if(($worD[0] =~ /\bthe\b/i and $worD[1] =~ /\b$Link_appos\b/) or $worD[0] =~ /\b$Link_appos\b/i)
			{
				print"同位语	1";
				$result[$nums] = "AC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
l|stole|stolen|sung|take|taken|taught|teach|tell|threw|throw|thrown|told|took|write|written|wrote)s?$/;
	...
	if(($worD[$sbarallword[0] -2] !~ /\,/ and  $worD[$sbarallword[0]-2] !~ /$Link_appos/ and scalar(@sbarallword) > 0.3) and $attr_introflag == 1 and ($worD[$sbarallword[0] -3] !~ /\,/) and  ($sbarallword[0]-1) != 0)
	{
			...
			if(scalar(@sbarsschild) > 0.3 and ($poS[$sbarallword[0] -2] =~ /^NN|^PRP/ or $worD[$sbarallword[0] -2] =~ /\b(those|these|this|this)\b/i))
			{
				my $Vp = ();
				my $Np = ();
				foreach(@sbarsschild)
	...
	if(scalar(@intro) == 0 and $poS[$sbarallword[0-2]] =~ /^NN/)
	{
		print"定语从句	12";
		$result[$nums] = "XATC";
		print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
sub MNRadvc
{
	...
	my @pOs = Pos();
	my @poS = @{$pOs[$nums]};
	if($worD[$intro[0]-1] =~ /\bthat\b/ and $worD[$intro[0]-2] =~ /\bway\b/ and $worD[$intro[0]-3] =~ /\bthe\b/)
	{
		print"方式状语从句	1";
	    $result[$nums] = "MNRADVC";
	    print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
	...
	#print $
	if($worD[$sbarallword[0]-1] =~ /\bwhat\b/i)
	{
		my $leftflag = 0;
		my $rightflag = 0;
		print "GGGG\n";
		for(my $i = 0;$i<$sbarallword[0];$i++){if($worD[$i] =~ /\bis\b/i and $worD[$i+1] =~ /\bto\b/i) {$leftflag = 1;}}
		for(my $i = $sbarallword[0];$i<scalar(@worD);$i++){if($worD[$i] =~ /\bis\b/i and $worD[$i+1] =~ /\bto\b/i) {$rightflag = 1;}}
		if($leftflag == 1 and $rightflag == 1){print"方式状语从句	1";
	    $result[$nums] = "MNRADVC";
	    print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
	    return 2;}
	}
...
######################################################################################################################################################################
 sub PreADVC
 {
	...
	if(scalar(@intro) == 0)
	{
		if($worD[$intro[0] -1] =~ /\bfear\b/ and $worD[$intro[0] - 2] =~ /for/){print"目的状语从句	2";$result[$nums] = "PADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;}
		if($worD[$intro[0] -1] =~ /\border\b/ and $worD[$intro[0] - 2] =~ /in/){print"目的状语从句	3";$result[$nums] = "PADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;}
		if($worD[$intro[0] -1] =~ /\bhope\b/ and $worD[$intro[0] - 2] =~ /the/ and $worD[$intro[0] - 3] =~ /in/){print"目的状语从句	5";$result[$nums] = "PADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;}
		if($worD[$intro[0] -1] =~ /\bend\b/ and $worD[$intro[0] - 2] =~ /the/ and $worD[$intro[0] - 3] =~ /to/){print"目的状语从句	6";$result[$nums] = "PADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;}
	}
	if(scalar(@intro) == 1)
	{
	...
	if( scalar(@sbarsschild) > 0.3 and $tmpmaintruck == 1)
		{
			...
			if(defined($Np))
			{
				...
				if($worD[$intro[0]-1] =~ /^(whenever)$/ and $intro[0] >2)
				{
					print"时间状语从句	whenever";
					$result[$nums] = "TADVC";
					print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
					return 2;
				}
...
sub Reason_Adv_Clasuse
{
	...
	{
		if($worD[$intro[0]-1] =~ /^because$/i or $worD[$intro[0]] =~ /\bbecause\b/i)
		{
			print"原因状语从句	1";$result[$nums] = "RADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;
		}
		if($worD[$intro[0]] =~ /\bthat\b/ and $worD[$intro[0]-1] =~ /\b(Seeing|considering|now$|given)\b/i)
		{
			print"原因状语从句	1";$result[$nums] = "RADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;
		}
		if($worD[$intro[0]-1] =~ /\bthat\b/ and $worD[$intro[0]-2] =~ /\b(Seeing|considering|now$|given)\b/i)
		{
			print"原因状语从句	1";$result[$nums] = "RADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;
		}
	}
...
sub WhereADVC
{
	...
	if( scalar(@sbarsschild) > 0.3 and $tmpmaintruck == 1)
	{
		if(scalar(@intro) <= 2)
		{
			if($worD[$sbarallword[0]-2] =~ /^(anywhere|everywhere|wherever)$/i or $worD[$sbarallword[0]-1] =~ /^(anywhere|everywhere|wherever)$/i)
			{
				print"地点状语从句	1";
				$result[$nums] = "WADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
sub ConcessionADVC
{
	...
	if(scalar(@sbarsschild) > 0.3)
	{
		...
		if(scalar(@intro) == 2 and $worD[$intro[0]-1] =~ /$qstwordever/i)
		{
					print"让步状语从句	5";
					$result[$nums] = "CCSADVC";
					print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
		...
		if( $worD[$intro[0]-1] =~ /^(whenever|wherever)$/i and $intro[0] <= 2)
		{
					print"让步状语从句 whenever??	5";
					$result[$nums] = "CCSADVC";
					print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
					return 2;
		}
...
sub RslADVC
{
	...
	if($tmpmaintruck == 1)
	{
		if(scalar(@intro) == 1)
		{
			if($worD[$intro[0] - 1] =~ /^that$/ and $worD[$intro[0] -2] =~ /\bso\b/i and $worD[$intro[0] -3] !~ /,/)
			{
				my $extMd = 0;
				for(my $i = $sbarallword[0] -1;$i<$sbarallword[scalar[@sbarallword]-1];$i++)
				{
				...
				if($extMd = 1)
				{
					print"目的状语从句 	2";$result[$nums] = "PADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;
				}
				else{print"结果状语从句 	2";$result[$nums] = "RSLADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;}
			}
		}
		if(scalar(@intro) == 2)
		{
			if($worD[$intro[0] - 1] =~ /\bso\b/i and $worD[$intro[0]] =~ /\bthat\b/i)
			{
				my $extMd = 0;
				for(my $i = $sbarallword[0] -1;$i<$sbarallword[scalar[@sbarallword]-1];$i++)
				{
		...
		if(scalar(@intro) == 1)
		{
			...
			if($worD[$intro[0] - 1] =~ /^that$/ and $worD[$intro[0] - 2] =~ /^(degree|extent)$/ and $worD[$intro[0] - 3] =~ /^(the)$/ and $worD[$intro[0] - 4] =~ /^(to)$/i)
			{
...
sub ThSComplex
{
	...
	if(scalar(@{$sub_location[$nums]}) > 1.9)
	{
		for(my $i = @{$sub_location[$nums]}[0];$i < @{$sub_location[$nums]}[1];$i++)
		{
			...
			if($worD[$i] =~ /^condition$/i and $worD[$i-1] =~ /^on$/)
			{
				print"Ths	条件状语从句4";
				$result[$nums] = "CADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /^for$/i and $worD[$i+1] =~ /\bfear\b/i)
			{
				print"Ths	目的状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /^in$/i and $worD[$i+1] =~ /\border\b/i)
			{
				print"Ths	目的状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /^in$/i and $worD[$i+1] =~ /\bthe\b/i and $worD[$i+1] =~ /\bhope\b/)
			{
				print"Ths	目的状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /\bto\b/i and $worD[$i+1] =~ /\bthe\b/i and $worD[$i+1] =~ /\bend\b/)
			{
				print"Ths	目的状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /anywhere|everywhere|wherever/i)
			{
				print"Ths	地点状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			...
			if($worD[$i] =~ /\bthe\b/ and $worD[$i+1] =~ /\bway\b/)
			{
				print"Ths	地点状语从句";
				$result[$nums] = "PADVC";
				print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
sub As
{
	...
	if($asflag == 1)
	{
		...
		if(($worD[$intro[0]-2] =~ /,/ or $worD[$sbarallword[scalar(@sbarallword)-1]] =~ /,/) and $poS[$sbarallword[scalar(@sbarallword)-1]-1] =~ /^VB/ and $poS[$sbarallword[scalar(@sbarallword)-1]-1] !~ /$Vi/)
		{
			#print $worD[$sbarallword[scalar(@sbarallword)-1]];
			print "非限定定语从句  2";
			$result[$nums] = "NXATC";
		...
		if($worD[$intro[0]-1] =~ /\bas\b/i)
		{
			for(my $i = 0;$i<$intro[0];$i++){if($worD[$i] =~ /\b(as|so)\b/ and $poS[$i+1] =~ /(RB|JJ)/ and $worD[$i+1] !~ /^(soon|long)$/i){print "比较状语从句  1";$result[$nums] = "CMPADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;} }
		}
		...
		if($worD[$intro[0]-1] =~ /\bas\b/ and$worD[$intro[0]] =~ /\b(if|though)\b/)
		{
			print "让步状语从句  8";
			$result[$nums] = "MNRADVC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
		...
		if($worD[$intro[0]-1] =~ /\bjust\b/i and $worD[$intro[0]] =~ /\bas\b/ and $worD[$intro[0]+1] =~ /\bif\b/)
		{
			print "让步状语从句  8";
			$result[$nums] = "MNRADVC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
		...
		for(my $ii=$sbarallword[scalar(@sbarallword)-1];$ii < scalar(@worD);$ii++)
		{
			if($worD[$ii] =~ /\bso\b/i){print "让步状语从句  8";
			$result[$nums] = "MNRADVC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
			return 2;}
		}
...
sub SinceIf
{
	...
	if($ifflag == 1)
	{
		if($worD[$iflocate-1] !~ /\b(even|as)\b/ and $poS[$iflocate-1] =~ /VB/ and $worD[$iflocate-1] !~ /$Vi/)
		{
			print "宾语从句  3";
			$result[$nums] = "OC";
			print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";
...
sub CmpAdvc
{
	...
	if($thanflag == 1)
	{
		if($worD[$intro[0]-1] =~ /\bthan\b/i)
		{
			print "比较状语从句  2";$result[$nums] = "CMPADVC";print "第",$nums+1,"句是复杂句 : @{$Word[$nums]}\n";return 2;
		}
	}
}
